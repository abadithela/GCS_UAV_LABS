...
/**
 * @author dcollins
 * @version $Id: SlideShowAnnotationController.java 10631 2009-04-29 03:02:34Z dcollins $
 */
...
public class SlideShowAnnotationController extends DialogAnnotationController
{
    ...
    public SlideShowAnnotationController(WorldWindow worldWindow, SlideShowAnnotation annotation,
        java.util.List<? extends Object> imageSources)
    {
    ...
    public void setImageSources(java.util.List<? extends Object> imageSources)
    {
    ...
    protected void doSetImage(Object source, int index)
    {
        ...
        slideShowAnnotation.getImageAnnotation().setImageSource(source);
    ...
    {
        java.awt.image.BufferedImage image = this.getImage(source);
        if (image != null)
        {
    ...
    protected void doRetrieveAndSetImage(Object source, final int index)
    {
        ...
        final Object image = this.readImage(source);
    ...
    protected java.awt.image.BufferedImage readImage(Object source)
    {
        ImageIOReader reader = new ImageIOReader();
        try
        {
            java.awt.image.BufferedImage image = reader.read(source);
            this.putImage(source, image);
            return image;
        }
        catch (Exception e)
        {
            String message = Logging.getMessage("generic.ExceptionWhileRequestingImage", source);
    ...
    protected java.awt.image.BufferedImage getImage(Object source)
    {
        return (java.awt.image.BufferedImage) WorldWind.getMemoryCache(BUFFERED_IMAGE_CACHE_NAME).getObject(source);
    }

    protected boolean putImage(Object source, java.awt.image.BufferedImage image)
    {
        long sizeInBytes = ImageUtil.computeSizeInBytes(image);
