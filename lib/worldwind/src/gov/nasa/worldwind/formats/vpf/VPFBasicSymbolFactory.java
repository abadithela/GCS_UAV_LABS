...
/**
 * @author dcollins
 * @version $Id: VPFBasicSymbolFactory.java 12507 2009-08-21 07:30:14Z dcollins $
 */
public class VPFBasicSymbolFactory implements VPFSymbolFactory
{
        ...
        Collection<VPFSymbol> outCollection)
    {
        ...
        for (VPFFeature subFeature : feature)
        {
            ...
            for (int id : subFeature.getPrimitiveIds())
            {
                ...
                {
                    VecBuffer coords = combinedCoords.getSubBuffer(id);
                    if (coords != null)
                    {
    ...
    protected void addTextSymbol(CombinedFeature feature, VPFSymbolAttributes attr, Collection<VPFSymbol> outCollection)
    {
        String primitiveName = feature.getFeatureClass().getPrimitiveTableName();
        CompoundVecBuffer combinedCoords = this.primitiveData.getPrimitiveCoords(primitiveName);
        CompoundStringBuffer combinedStrings = this.primitiveData.getPrimitiveStrings(primitiveName);

        // Construct a renderable object for the first text symbol.
        for (int id : feature.getPrimitiveIds())
        {
            VecBuffer coords = combinedCoords.getSubBuffer(id);
            CharSequence text = combinedStrings.getSubBuffer(id);
    ...
    protected LatLon computeLabelLocation(VPFSymbolAttributes.LabelAttributes attr, VPFFeature feature)
    {
        ...
        // If we are given label offset parameters and a Globe context, then compute the offset label location.
        if (attr.getOffset() != 0 && attr.getOffsetAngle() != null && this.getGlobe() != null)
        {
            // Multiply offset in map units (millimeters) by the map scale to determine a real world offset in meters,
            // divide by the Globe's radius to get offset length in arc degrees, then compute the final label location
            // using the offset azimuth and offset arc length.

            double mapScale = feature.getFeatureClass().getCoverage().getLibrary().getMapScale();
            Angle offsetLength = Angle.fromRadians(
                mapScale * (attr.getOffset() / 1000d) / this.getGlobe().getRadiusAt(location));
            location = LatLon.greatCircleEndPosition(location, attr.getOffsetAngle(), offsetLength);
        }
