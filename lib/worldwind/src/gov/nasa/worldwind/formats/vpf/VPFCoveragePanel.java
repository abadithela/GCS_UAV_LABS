...
public class VPFCoveragePanel extends JPanel
{
    ...
    public void addLibrary(VPFDatabase db, VPFLibrary lib)
    {
        ...
        this.libraryTabbedPane.setToolTipTextAt(index, lib.getStringValue(AVKey.DESCRIPTION));
    ...
    protected void fillLegendPanel()
    {
        ...
        for (SymbolInfo info : symbolInfo)
        {
            ...
            if (coverageName == null || !info.getFeatureClass().getCoverage().getName().equals(coverageName))
            {
                ...
                JLabel label = new JLabel(info.getFeatureClass().getCoverage().getStringValue(AVKey.DISPLAY_NAME));
    ...
    private class LibraryAction extends AbstractAction
    {
        ...
        {
            super("Show Library");
            this.putValue(Action.SHORT_DESCRIPTION, "Show " + library.getStringValue(AVKey.DISPLAY_NAME));
            this.db = db;
    ...
    private class CoverageAction extends AbstractAction implements PropertyChangeListener
    {
        ...
        {
            super(coverage.getStringValue(AVKey.DISPLAY_NAME));
            this.putValue(Action.SHORT_DESCRIPTION, "Show " + coverage.getStringValue(AVKey.DESCRIPTION));
            this.db = db;
    ...
    private class ZoomAction extends AbstractAction
    {
        ...
        public void actionPerformed(ActionEvent actionEvent)
        {
            ...
            if (view instanceof OrbitView)
            {
                ...
                double zoom = zoomForSector(view, globe, wwd.getSceneController().getVerticalExaggeration(), sector);
                view.goTo(new Position(sector.getCentroid(), 0),  zoom);
                wwd.redraw();
            }
        }
    }

    public static double zoomForSector(View view, Globe globe, double verticalExaggeration, Sector sector)
    {
        Extent extent = globe.computeBoundingCylinder(verticalExaggeration, sector);
        if (extent == null)
            return -1;

        return extent.getRadius() / view.getFieldOfView().tanHalfAngle();
    }
}
