 ...
 *
 * @author dcollins
 * @version $Id: VPFDatabase.java 12476 2009-08-18 07:56:38Z dcollins $
 */
public class VPFDatabase extends AVListImpl
{
    ...
    /**
     * Constructs a VPF Database, initializing the Library Attribute Table and Database Header Table.
     *
     * @param filePath the directory path under which the Database Header Table resides.
     *
     * @throws IllegalArgumentException if the file path is null.
     */
    public VPFDatabase(String filePath)
    {
        if (filePath == null)
        {
    ...
    public static VPFDatabase fromFile(String filePath)
    {
        ...
        if (record != null)
        {
            VPFUtils.checkAndSetValue(record, "database_name", AVKey.TITLE, database);
            VPFUtils.checkAndSetValue(record, "database_desc", AVKey.DESCRIPTION, database);
            database.setValue(AVKey.DISPLAY_NAME, database.getValue(AVKey.TITLE));
        }
