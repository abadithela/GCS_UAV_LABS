 ...
 *
 * @author dcollins
 * @version $Id: VPFLibrary.java 12476 2009-08-18 07:56:38Z dcollins $
 */
public class VPFLibrary extends AVListImpl
{
    ...
    /**
     * Constructs a VPF Database, initializing the Library Header Table, Geographic Reference Table, and Coverage
     * Attribute Table to default values. The Data Quality Table and Tile Reference Coverage is null.
     *
     * @param database the Database which this Library resides in.
     * @param name     the library's name.
     *
     * @throws IllegalArgumentException if either the database, name, or bounding box is null.
     */
    public VPFLibrary(VPFDatabase database, String name)
    {
        if (database == null)
        {
    ...
    public static VPFLibrary fromFile(VPFDatabase database, String name)
    {
        ...
        VPFTableReader reader = new VPFTableReader();

        VPFBufferedRecordData lht = reader.read(new File(file, VPFConstants.LIBRARY_HEADER_TABLE));
        if (lht == null)
        {
        ...
        VPFBufferedRecordData cat = reader.read(new File(file, VPFConstants.COVERAGE_ATTRIBUTE_TABLE));
        if (cat == null)
        {
        ...
        VPFBufferedRecordData grt = reader.read(new File(file, VPFConstants.GEOGRAPHIC_REFERENCE_TABLE));
        if (grt == null)
        {
        ...
        VPFLibrary library = new VPFLibrary(database, name);
        ...
        record = library.getLibraryHeaderTable().getRecord(1);
        if (record != null)
        {
            VPFUtils.checkAndSetValue(record, "library_name", AVKey.TITLE, library);
            VPFUtils.checkAndSetValue(record, "description", AVKey.DESCRIPTION, library);
            library.setValue(AVKey.DISPLAY_NAME, library.getValue(AVKey.TITLE));
        }

        // Coverage meta attributes.
