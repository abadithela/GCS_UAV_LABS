 ...
 *
 * @author dcollins
 * @version $Id: BasicLayerFactory.java 12587 2009-09-14 21:18:02Z tgaskins $
 */
public class BasicLayerFactory extends BasicFactory
{
    ...
    protected Object doCreateFromElement(Element domElement, AVList params) throws Exception
    {
        ...
        if (domElement.getLocalName().equals("Layer"))
    ...
    protected LayerList[] createLayerLists(Element[] elements, AVList params)
    {
        ...
        for (Element element : elements)
        {
            try
            {
                ...
                {
                    LayerList list = (LayerList) this.createFromConfigSource(href, params);
                ...
                if (children != null && children.length > 0)
                {
                    ...
                    if (list != null && list.size() > 0)
                    {
                        ...
                        if (title != null && title.length() > 0)
                            list.setValue(AVKey.TITLE, title);
                    }
                }
            }
            catch (Exception e)
    ...
    protected Layer createFromLayerDocument(Element domElement, AVList params)
    {
        ...
        if (className != null && className.length() > 0)
            return (Layer) WorldWind.createComponent(className);
        ...
        if (href != null && href.length() > 0)
        {
            Layer layer = (Layer) this.createFromConfigSource(href, params);
            if (layer != null)
            {
