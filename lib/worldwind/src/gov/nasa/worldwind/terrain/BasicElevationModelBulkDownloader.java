 ...
 *
 * @author tag
 * @version $Id: BasicElevationModelBulkDownloader.java 12546 2009-09-03 05:36:57Z tgaskins $
 */
public class BasicElevationModelBulkDownloader extends BulkRetrievalThread
{
    ...
    public BasicElevationModelBulkDownloader(BasicElevationModel elevationModel, Sector sector, double resolution)
    {
        ...
        super(elevationModel, sector, resolution, elevationModel.getDataFileStore());
    ...
    {
        // Arguments checked in parent constructor
        super(elevationModel, sector, resolution, fileStore);
        ...
        try
        {
            ...
            for (int levelNumber = 0; levelNumber <= this.level; levelNumber++)
            {
                ...
                for (Sector region : regions)
                {
                    ...
                    missingTiles = getMissingTilesInSector(region, levelNumber);
                    foundMissingCount += missingTiles.size();
    ...
    protected long getEstimatedMissingDataSize()
    {
        ...
        Sector[] regions = computeRegions(sector, maxLevel, 36); // max 6x6 tiles per region
        int samples = 6;
        int regionMissing = 0;
        int regionCount = 0;
        try
        {
            ...
            {
                regionCount = this.countTilesInSector(sector, maxLevel);
                regionMissing = this.getMissingTilesInSector(sector, maxLevel).size();
            }
            else
            {
                ...
                while (samples > 0)
                {
                    ...
                    if (region != null)
                    {
                        ...
                        regionMissing += this.getMissingTilesInSector(region, maxLevel).size();
                        regions[i] = null;
                        samples--;
                    }
                }
    ...
    protected int countTilesInSector(Sector sector, int levelNumber)
    {
        if (sector == null)
        {
    ...
    {
        int tileCount = countTilesInSector(sector, levelNumber);
